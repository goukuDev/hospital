...
export default forwardRef(function Index(props, ref) {
    ...
    const loadReportInfo = useCallback((info = reportInfo)=>{
        let copy = isCopy(info);
        content.current = copy? Object.assign({},content.current,info.content):Object.assign({},content.current,{report: [],common: {}});
    ...
    const getMainReportType = (arr) => {
        let tmpArr = (arr || [])
            .filter(o => o.modifyDate && o)
            .map(o => new Date(o.modifyDate).getTime());
        ...
        if(common.subsequentDoc){
            Confirm({
                ...
                onOk:e=>{
                    ...
                    api(`report/alter_report_review?report_id=${reportId}`, {review: 1})
                        .then(({ code, message, data }) => {
                        if (code === CODE.SUCCESS) {
                            setControlObj(Object.assign({},controlObj,{
                                CANCEL: false,
                                PRINT: false,
                                AUDIT: true,
                                readOnly: true
            ...
            <div className={style.fl}>
                <div className={style.top}>
                    ...
                    <div className={style.templat}>
                        ...
                        />
                        <Lung
                            visible={mainReportType === "lung"}
                            updateLung={value => updateReport("report", value)}
                        />
                    </div>
                    <div className={style.footer}>
                        <div style={{overflow: "hidden"}}>
                            <div className={style["input-box"]}>
                                初诊医生：
                                <div className={`${style.text} ${controlObj.readOnly?style.disable:''}`}>{common.firstDoc}</div>
                            </div>
                            <div className={style["input-box"]}>
                                复诊医生:
                                <Select
                            ...
                            <div className={style["input-box"]}>
                                审核医生：
                                <div className={`${style.text} ${controlObj.readOnly?style.disable:''}`}>{common.auditDoc}</div>
                            </div>
                        </div>
                        <div style={{overflow: "hidden"}}>
                            <div className={style["input-box"]}>
                ...
                <div className={style.bottom}>
                    <button disabled={controlObj.readOnly} onClick={save}> 保存 </button>
                    <button disabled={controlObj.AUDIT} onClick={audit}>审核</button>
                    <button disabled={controlObj.CANCEL} onClick={cancelAudit}>取消审核</button>
                    <button disabled={controlObj.PRINT} onClick={print}>打印</button>
                </div>
            </div>
