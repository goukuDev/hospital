...
export default function Index(props) {
            ...
            render: value => (
                <React.Fragment>
                    <span
                        className={style.dot}
                        style={{
                            background: { 4: '#6FC831', 3: '#F4BB00' }[value],
                        }}
                    />
                    {status[value]}
                </React.Fragment>
    ...
    const [filters, setFilters] = useState({ status: [3, 4] });
    ...
    const getFacilities = e => {
        return api('immuno/get_slices_equip', { status: JSON.stringify([3,4]) }).then(({ code, data, message }) => {
    ...
    useEffect(() => {
        let data = {
            filters: JSON.stringify({ status: [3, 4] }),
            sort_key:'slice_time',
            sort_value:'1',
        };
        getCaseRecords({
    ...
    const handleSearch = (pagination, filters) => {
        ...
        if (dateRange.length) {
            filters.slice_time = [fmtDate(dateRange[0]), fmtDate(dateRange[1])]
        } else {
            if (filters.slice_time) delete filters.slice_time;
        }
        ...
        filters.status = status && status.length ? status : inputValue ? '[]' : [3, 4];
        let data = {
