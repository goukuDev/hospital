...
function Add(props){
    ...
    const addBrand = () => {
        ...
        api('immu_manage/add_facility',data)
...
export default function Index(props){
    ...
    const getDeviceList = () => {
        api('immu_manage/get_facilities_detail')
        .then(({code,data}) => {
            if(code === CODE.SUCCESS){
    ...
    const modifyStatus = (id) => {
        api(`immu_manage/alter_facility_status?facility_id=${id}`)
        .then(({code}) => {
            if(code === CODE.SUCCESS){
                getDeviceList();
            }
    ...
    const editRecord = (id,flag) => {
        if(flag){
            ...
            api('immu_manage/alter_facility',data)
            .then(({code,message}) => {
                if(code === CODE.SUCCESS){
                    let arr = deviceList.map(o => {
                        if(o.facility_id === id){
