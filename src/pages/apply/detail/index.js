...
export default function Detail(props) {
    ...
    const getApplyData = pis_apply_id =>
        new Promise(resolve =>
            api('records/get_appli', {pis_apply_id}).then(({code, data}) => {
    ...
    const addSample = async params => {
        // 新建状态，第一次添加标本时，需要先保存基本信息
        if ([ACTIONS.NEW, ACTIONS.COPY].includes(action) && sampleInfo.length === 0) {
    ...
    useEffect(() => {
        async function fetchData() {
            // 复制时需要获取父本的基本信息
            const pis_apply_id = ACTIONS.COPY === action ? regionApplyId : applyId;
    ...
    const handleSampleData = (data = []) => {
        return data.map(o => {
            if (o.status === SAMPLE_STATUS.REJECTED) {
                return Object.assign({}, o, {isRejected: true, disabled: true});
            }
            return o;
        });
    };
    ...
    const save = ({showSuccessMessage = true} = {}) =>
        new Promise((resolve, reject) => {
            ...
            api('records/save_appli', {record: JSON.stringify(params)}).then(({code, data}) => {
                if (CODE.SUCCESS === code) {
    ...
    return (
        <Transition>
            <div className={style.container}>
                ...
                <img src={require('@images/close.png')} alt='' className={style.close} onClick={handleClose} />
                <div className={style.content}>
                    ...
                    />
                    <ApplyInfo applyMessage={applyMessage} updateState={value => setApplyMessage(value)} />
                    <SampleInfo
                        basicMessage={basicMessage}
                        ...
                        updateState={newState => setApplyMessage({...applyMessage, ...newState})}
                    />
                </div>
                <div
                    className={`${style.saveBtn} ${canSave ? style.active : ''}`}
